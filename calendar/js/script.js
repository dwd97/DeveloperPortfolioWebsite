document.addEventListener('DOMContentLoaded', function () {
  ...
  const calendar = new FullCalendar.Calendar(calendarEl, {
    ...
    header: {
      right: 'prev,next today customButton',
      center: 'title',
      left: ''
    },
  ...
  // Ensure the addEventButton is selected after the calendar is rendered
  const addEventButton = document.querySelector('.fc-customButton-button');
  addEventButton.style.display = 'none';

  // Add keyboard shortcut (Command + Shift + E) for "Add Event" button
...
function promptForPassword(callback) {
  const password = prompt('Enter password to access the context menu:');
  if (password === 'yourPassword') { // Replace 'yourPassword' with your actual password
    callback();
  } else {
    alert('Incorrect password');
